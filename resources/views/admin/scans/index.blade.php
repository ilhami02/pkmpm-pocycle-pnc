...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
    <div class="bg-white rounded-2xl border border-earth-200 p-5 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-2xl font-bold text-earth-800">{{ $stats['total'] }}</p>
    ...
    <div class="bg-white rounded-2xl border border-amber-200 p-5 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-2xl font-bold text-amber-700">{{ $stats['needs_stirring'] }}</p>
    ...
    <div class="bg-white rounded-2xl border border-red-200 p-5 shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <p class="text-2xl font-bold text-red-700">{{ $stats['contaminated'] }}</p>
...
<div class="bg-white rounded-2xl border border-earth-200 shadow-sm overflow-hidden">
    ...
    <div class="p-6 border-b border-earth-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <form method="GET" action="{{ route('admin.scans.index') }}" class="w-full sm:w-96 relative">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama user atau warna..."
        ...
        <div class="flex items-center gap-2 flex-wrap">
            <a href="{{ route('admin.scans.index', array_merge(request()->except('status', 'page'))) }}"
               class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {{ !request('status') ? 'bg-earth-800 text-white' : 'bg-earth-100 text-earth-600 hover:bg-earth-200' }}">
                Semua
            </a>
            <a href="{{ route('admin.scans.index', array_merge(request()->except('page'), ['status' => 'normal'])) }}"
               class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {{ request('status') === 'normal' ? 'bg-green-600 text-white' : 'bg-green-50 text-green-700 hover:bg-green-100' }}">
                ✅ Normal
            </a>
            <a href="{{ route('admin.scans.index', array_merge(request()->except('page'), ['status' => 'needs_stirring'])) }}"
               class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {{ request('status') === 'needs_stirring' ? 'bg-amber-600 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100' }}">
                ⚠️ Perlu Diaduk
            </a>
            <a href="{{ route('admin.scans.index', array_merge(request()->except('page'), ['status' => 'contaminated'])) }}"
               class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {{ request('status') === 'contaminated' ? 'bg-red-600 text-white' : 'bg-red-50 text-red-700 hover:bg-red-100' }}">
                🚫 Terkontaminasi
            </a>
        </div>
    </div>
            ...
            <tbody class="divide-y divide-earth-100 text-sm text-earth-800">
                @forelse($scans as $scan)
                    <tr class="hover:bg-earth-50 transition-colors">
                        ...
                        {{-- Tanggal --}}
                        <td class="px-6 py-4">
                            <div class="text-earth-800">{{ $scan->created_at->format('d M Y') }}</div>
                            <div class="text-xs text-earth-500">{{ $scan->created_at->format('H:i') }} WIB</div>
                        </td>

                        {{-- Status --}}
                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-md border {{ $scan->status_color }}">
                                {{ $scan->status_label }}
                            </span>
                        </td>
                        ...
                        {{-- Suhu --}}
                        <td class="px-6 py-4">
                            <span class="font-medium text-earth-800">{{ $scan->temperature }}°C</span>
