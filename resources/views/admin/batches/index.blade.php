...
@section('title', 'Galon Aktif (On-Progress)')
...
<div class="bg-white rounded-2xl border border-earth-200 shadow-sm overflow-hidden mb-6">
    ...
    <div class="p-6 border-b border-earth-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <form method="GET" action="{{ route('admin.batches.active') }}" class="w-full sm:w-80 relative">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama user atau galon..."
            ...
            <tbody class="divide-y divide-earth-100 text-sm text-earth-800">
                @forelse($batches as $batch)
                    ...
                    <tr class="hover:bg-earth-50 transition-colors">
                        ...
                        {{-- Umur --}}
                        <td class="px-6 py-4">
                            <span class="font-medium text-earth-800">Hari ke-{{ $day }}</span>
                            <div class="text-xs text-earth-500 mt-0.5">Minggu ke-{{ $week }}</div>
                        </td>

                        {{-- Update Terakhir --}}
                        <td class="px-6 py-4">
                            <div class="text-earth-800">{{ $batch->updated_at->format('d M Y') }}</div>
                            <div class="text-xs text-earth-500 mt-0.5">{{ $batch->updated_at->diffForHumans() }}</div>
                        </td>

                        {{-- Status Scan Terakhir --}}
                        <td class="px-6 py-4">
                            @if($latestScan)
                                <span class="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-md border {{ $latestScan->status_color }}">
